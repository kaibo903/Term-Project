...
#### 行動裝置操作
- 在手機或小螢幕上，作業列表會自動切換為「卡片式」檢視，每張卡片列出所有工期與成本欄位，並保留「編輯／刪除」按鈕。
- 卡片內的欄位仍支援即時編輯，輸入框會自動放大且填滿整個欄位，避免誤觸。
- 若專案尚無作業，會顯示 `暫無作業資料` 的提示，操作邏輯與桌面版一致。
...
#### 行動裝置檢視
- 小螢幕會自動切換為卡片式列表，卡片中顯示專案名稱、描述、狀態與建立時間，並提供「管理作業／編輯／刪除」按鈕。
- 卡片操作順序與桌面版相同，所有按鈕皆可單手點擊；若沒有專案，畫面會顯示空狀態提示。
